...
func NewShader(vertexFmt, uniformFmt AttrFormat, vertexShader, fragmentShader string) (*Shader, error) {
	...
	// shader program
	{
		...
		gl.AttachShader(shader.Handle, vshader)
		gl.AttachShader(shader.Handle, fshader)
		gl.LinkProgram(shader.Handle)

		var success int32
		gl.GetProgramiv(shader.Handle, gl.LINK_STATUS, &success)
		if success == gl.FALSE {
			var logLen int32
			gl.GetProgramiv(shader.Handle, gl.INFO_LOG_LENGTH, &logLen)

			infoLog := make([]byte, logLen)
			gl.GetProgramInfoLog(shader.Handle, logLen, nil, &infoLog[0])
	...
	for i, uniform := range uniformFmt {
		loc := gl.GetUniformLocation(shader.Handle, gl.Str(uniform.Name+"\x00"))
