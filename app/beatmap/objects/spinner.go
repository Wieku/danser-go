...
func (self *Spinner) SetDifficulty(diff *difficulty.Difficulty) {
	...
	self.spinnerbonus = bass.NewSample("assets/sounds/spinnerbonus.wav")
...
func (self *Spinner) Draw(time int64, color mgl32.Vec4, batch *sprite.SpriteBatch) bool {
	...
	scoreFont.Draw(batch, self.ScaledWidth/2+139-scoreFont.GetWidth(48, rpmTxt), self.ScaledHeight-28, 48, rpmTxt)
...
func (self *Spinner) StartSpinSample() {
	if self.loopSample == 0 {
		self.loopSample = bass.NewSampleLoop("assets/sounds/spinnerspin.wav").Play()
	} else {
