...
func InitRenderer() {
	...
	fboSprite = sprites.NewSpriteSingle(&region, 0, bmath.NewVec2d(settings.Graphics.GetWidthF()/2, settings.Graphics.GetHeightF()/2), bmath.Origin.Centre)
...
func drawSlider(sprite *sprites.Sprite, text texture.Texture, color mgl32.Vec4, prev mgl32.Vec4, projection mgl32.Mat4) {
	...
	colorShader.SetUniform("position", mgl32.Vec2{sprite.GetPosition().X32(), sprite.GetPosition().Y32()})
