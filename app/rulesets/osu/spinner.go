		...
		if player.cursor.IsReplayFrame && time > int64(spinner.hitSpinner.GetStartTime()) && time < int64(spinner.hitSpinner.GetEndTime()) {
			...
			if state.rotationCount != state.lastRotationCount {
				...
				if state.scoringRotationCount > state.requirement+3 && (state.scoringRotationCount-(state.requirement+3))%2 == 0 {
					...
					spinner.ruleSet.SendResult(time, player.cursor, spinner, spinnerPosition.X, spinnerPosition.Y, SpinnerBonus, true, ComboResults.Hold)
				} else if state.scoringRotationCount > 1 && state.scoringRotationCount%2 == 0 {
					spinner.ruleSet.SendResult(time, player.cursor, spinner, spinnerPosition.X, spinnerPosition.Y, SpinnerPoints, true, ComboResults.Hold)
				} else if state.scoringRotationCount > 1 {
					spinner.ruleSet.SendResult(time, player.cursor, spinner, spinnerPosition.X, spinnerPosition.Y, SpinnerSpin, true, ComboResults.Hold)
				}
...
func (spinner *Spinner) UpdatePostFor(player *difficultyPlayer, time int64) bool {
	...
	if time >= int64(spinner.hitSpinner.GetEndTime()) && !state.finished {
		...
		spinner.ruleSet.SendResult(time, player.cursor, spinner, spinner.hitSpinner.GetPosition().X, spinner.hitSpinner.GetPosition().Y, hit, false, combo)
