		...
		if clicked && inRange {
			...
			if action == Click {
				...
				if hit != Ignore {
					...
					circle.ruleSet.SendResult(time, player.cursor, circle, position.X, position.Y, hit, false, combo)
...
func (circle *Circle) UpdatePostFor(player *difficultyPlayer, time int64) bool {
	...
	if time > int64(circle.hitCircle.GetEndTime())+player.diff.Hit50 && !state.isHit {
		position := circle.hitCircle.GetStackedPositionAtMod(float64(time), player.diff.Mods)
		circle.ruleSet.SendResult(time, player.cursor, circle, position.X, position.Y, Miss, false, ComboResults.Reset)
